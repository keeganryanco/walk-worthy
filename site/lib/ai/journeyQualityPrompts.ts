...
export function buildDevotionalCorePrompt(input: JourneyPackageRequest, repairNotes?: string): { system: string; user: string } {
  ...
  const system = [
    ...
    "Use this reflection shape without making it obvious: sentence 1 anchors in the selected Scripture; sentence 2 develops what the Scripture reveals; sentence 3 connects that truth to the user's journey; sentence 4 or 5 closes the thought with a grounded implication.",
  ...
  const user = JSON.stringify({
    ...
    instructions: [
      ...
      "Before returning JSON, privately check that all fields serve the same point, the reflection reads as one complete thought, the reflection contains no practical assignment or meta framing, the title is not generic, the prayer names concrete realities from the journey, and the arc fields are all present.",

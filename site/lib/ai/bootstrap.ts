...
function buildBootstrapPrompt(request: JourneyBootstrapRequest, repairNotes?: string): { system: string; user: string } {
  ...
  const system = [
    ...
    "reflectionThought must read as one coherent thought with a beginning, middle, and end: anchor in Scripture, develop what Scripture reveals, connect to the user's journey, then close with a grounded implication.",
